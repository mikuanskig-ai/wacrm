...
import type { DayKey } from "@/lib/delivery/business-hours";
...
export function ProductForm({
  ...
  // Per-weekday price override — empty string = no override that day.
  // Only populated days (non-empty, valid) end up in the saved
  // day_price_overrides JSONB; see buildPayload().
  const [dayPrices, setDayPrices] = useState<Partial<Record<DayKey, string>>>({});
  ...
  useEffect(() => {
    ...
    if (product) {
      ...
      const overrides = product.day_price_overrides ?? {};
      setDayPrices(
        Object.fromEntries(
          DAY_KEYS.filter((day) => typeof overrides[day] === "number").map((day) => [
            day,
            String(overrides[day]),
          ]),
        ),
      );
  ...
  function buildDayPriceOverrides(): Partial<Record<DayKey, number>> {
    const overrides: Partial<Record<DayKey, number>> = {};
    for (const day of DAY_KEYS) {
      const raw = dayPrices[day];
      if (raw === undefined || raw === "") continue;
      const value = parseFloat(raw);
      if (Number.isFinite(value) && value >= 0) overrides[day] = value;
    }
    return overrides;
  }
  ...
  function buildPayload() {
    return {
      ...
      day_price_overrides: buildDayPriceOverrides(),
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="grid gap-2 border-t border-border/50 pt-4">
              <Label className="text-muted-foreground">{t("dayPrices")}</Label>
              <p className="text-xs text-muted-foreground">{t("dayPricesDesc")}</p>
              <div className="space-y-1.5">
                {DAY_KEYS.map((day) => {
                  const override = dayPrices[day];
                  const hasOverride = override !== undefined;
                  return (
                    <div key={day} className="flex items-center gap-3 rounded-md border border-border p-2">
                      <span className="w-16 shrink-0 text-sm text-foreground">{tDay(day)}</span>
                      <Switch
                        checked={hasOverride}
                      ...
                      {hasOverride ? (
                        <Input
                          ...
                          value={override}
                          onChange={(e) =>
                            setDayPrices((prev) => ({ ...prev, [day]: e.target.value }))
                          }
                          placeholder="0.00"
                          className="h-8 border-border bg-muted text-foreground"
                        />
                      ) : (
                        <span className="text-xs text-muted-foreground">{t("dayPricesDefault")}</span>
                      )}
                    </div>
                  );
                })}
