...
const TRIGGER_OPTIONS: { value: AutomationTriggerType; deliveryOnly?: boolean }[] = [
  ...
  { value: "tag_added" },
  { value: "time_based" },
  { value: "order_created", deliveryOnly: true },
  { value: "order_status_changed", deliveryOnly: true },
  { value: "order_payment_status_changed", deliveryOnly: true },
]

const ORDER_STATUSES: DeliveryOrderStatus[] = [
  "draft",
...
const PAYMENT_STATUSES: DeliveryPaymentStatus[] = [
  "pending_payment",
  "approved",
  "rejected",
  "refunded",
...
function TriggerCard({
  ...
  t: ReturnType<typeof useTranslations>
  tDelivery: ReturnType<typeof useTranslations>
        ...
        {open && (
          <div className="space-y-3 border-t border-border px-4 py-3">
            <div>
              ...
              >
                {TRIGGER_OPTIONS.filter((o) => !o.deliveryOnly || deliveryEnabled).map((o) => (
            ...
            {type === "time_based" && (
              <div>
                ...
                <SchedulePicker
                  ...
                  onChange={(v) => onConfigChange({ ...config, schedule: v })}
                />
                <p className="mt-1 text-[11px] text-muted-foreground">
                  {t("scheduleHint")}
                </p>
            ...
            {type === "order_payment_status_changed" && (
              <div>
                ...
                <select
                  value={(config.payment_status as string) ?? ""}
                  onChange={(e) => onConfigChange({ ...config, payment_status: e.target.value })}
                  className={SELECT_CLASS}
                >
                  <option value="" disabled>
