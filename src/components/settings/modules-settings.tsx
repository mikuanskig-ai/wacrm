...
export function ModulesSettings() {
  ...
  // null = not loaded yet — stays permissive (don't grey out a switch
  // before we actually know the plan doesn't include it). The real
  // enforcement is the DB trigger (migration 063); this is UX only.
  ...
  useEffect(() => {
    ...
    fetch("/api/billing/current-plan")
      ...
      .then((data) => {
        if (!cancelled) setPlanModules(data.plan.enabledModules as string[]);
      })
      .catch(() => {
        // Best-effort — leave permissive on failure rather than
        // wrongly blocking every module toggle.
      });
    return () => {
      cancelled = true;
    };
  ...
  async function handleSave() {
    ...
    setSaving(true);
    // Marks the onboarding checklist's "business type" step done — the
    // act of deciding (with or without Delivery) is the signal, not
    // which way it went or how many times it's re-saved.
    const { error } = await supabase
      .from("accounts")
      .update({
        enabled_modules: enabled,
        onboarding_business_type_at: new Date().toISOString(),
      })
      .eq("id", accountId);
    if (error) {
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3">
            {MODULE_KEYS.map((key) => {
              ...
              const grantedByPlan = planModules === null || planModules.includes(key);
              const lockedByPlan = !enabled.includes(key) && !grantedByPlan;
              return (
                <div
                  key={key}
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="grid gap-0.5">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {lockedByPlan ? t("notInPlanHint") : t(`${key}Desc`)}
                      </span>
                    </div>
                  </div>
                  <Switch
                    ...
                    disabled={!canEditSettings || profileLoading || lockedByPlan}
                  />
                </div>
              );
            })}
