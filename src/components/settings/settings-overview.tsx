...
export function SettingsOverview({
  ...
  useEffect(() => {
    ...
    // WhatsApp connection status — a single row read, no external
    // round-trip needed now that the only channel is WuzAPI (its own
    // pairing state lives on the row's `status` column already).
    (async () => {
      setWhatsappLoading(true);
      const row = await supabase
        .from('whatsapp_config')
        .select('wuzapi_instance_name, status')
        .eq('account_id', acctId)
        .maybeSingle();
      if (cancelled) return;
      setWhatsapp({
        configured: !!row.data?.wuzapi_instance_name,
        connected: row.data?.status === 'connected',
      });
