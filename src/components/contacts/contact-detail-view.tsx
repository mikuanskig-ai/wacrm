...
import type { Contact, Tag, ContactTag, ContactNote, CustomField, ContactCustomValue, Deal, DeliveryOrder } from '@/types';
import { OrderStatusBadge } from '@/components/delivery/order-status-badge';
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
export function ContactDetailView({
  ...
  // Orders tab (Delivery module, opt-in)
  ...
  const fetchOrders = useCallback(async () => {
    if (!contactId || !deliveryEnabled) return;
  ...
  }, [contactId, deliveryEnabled, supabase]);
  ...
  }, [open, contactId, fetchContact, fetchTags, fetchNotes, fetchCustomFields, fetchDeals, fetchOrders]);
        ...
        {loading || !contact ? (
          ...
          <div className="flex flex-col h-full">
            ...
            <SheetHeader className="p-4 border-b border-border/50">
              <div className="flex items-center gap-3">
                <Avatar className="size-12 bg-muted border border-border">
                  <AvatarImage src={contact.avatar_url ?? undefined} alt={contact.name ?? contact.phone} />
            ...
            <Tabs defaultValue="details" className="flex-1 flex flex-col min-h-0">
              ...
              {deliveryEnabled && (
                <TabsContent value="orders" className="flex-1 overflow-y-auto px-4 py-3">
                  {loadingOrders ? (
                    <div className="flex items-center justify-center py-8">
                      <Loader2 className="size-5 animate-spin text-primary" />
                    </div>
                  ) : orders.length === 0 ? (
                    <p className="text-xs text-muted-foreground">{t('ordersTab.noOrders')}</p>
                  ) : (
                    <div className="space-y-2">
                      {orders.map((order) => (
                        <div
                        ...
                        >
                          ...
                          <div className="mt-1.5 flex items-center justify-between text-xs text-muted-foreground">
                            <span>{order.items?.length ?? 0} {t('ordersTab.items')}</span>
