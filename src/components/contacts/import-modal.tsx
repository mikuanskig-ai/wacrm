...
import { autoAddContactsToPipelines, type AutoAddContact } from '@/lib/pipelines/auto-add';
...
export function ImportModal({
  ...
  async function handleImport() {
    ...
    try {
      ...
      const createdContacts: AutoAddContact[] = [];
      ...
      for (let i = 0; i < toInsert.length; i += chunkSize) {
        ...
        if (error) {
          ...
          for (let j = 0; j < inserted.length; j++) {
            ...
            if (source.tagNames.length === 0) continue;
            tagAssignments.push({
      ...
      // 4b) Auto-add every newly-imported contact into any pipeline with
      // auto-add turned on — one shared lookup per import, not per row.
      if (createdContacts.length > 0) {
        await autoAddContactsToPipelines(supabase, accountId, user.id, createdContacts);
      }
