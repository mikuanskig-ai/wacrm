...
//
// Note there's no "Events / Triggers" category: in ZonTalk a flow is
...
export type NodeCategory = 'messaging' | 'logic' | 'commerce' | 'flow';
...
> = {
  ...
  add_order_item: {
    ...
    blurb: 'Customer picks a product + add-ons from the menu',
    category: 'commerce',
  },
  order_summary: {
    label: 'Order summary',
    icon: ReceiptText,
    color: 'text-lime-400',
    blurb: 'Shows the cart total, then adds another item or places the order',
    category: 'commerce',
  },
  numeric_menu: {
    label: 'Numeric menu',
    icon: ListOrdered,
    color: 'text-blue-400',
    blurb: 'Sends a numbered text menu — works on any channel, including WuzAPI',
    category: 'messaging',
  },
};
...
const NODE_HUE: Record<NodeType, { l: number; c: number; h: number }> = {
  ...
  add_order_item: { l: 0.68, c: 0.15, h: 45 }, // orange — cart/commerce
  order_summary: { l: 0.72, c: 0.14, h: 125 }, // lime — checkout
  numeric_menu: { l: 0.63, c: 0.15, h: 235 }, // blue — text-only menu
};
...
export function summarizeNode(
  ...
  switch (node.node_type) {
    ...
    case 'add_order_item': {
      const cartVarKey = typeof cfg.cart_var_key === 'string' ? cfg.cart_var_key : '';
      return cartVarKey ? `→ vars.${cartVarKey}` : null;
    }
    case 'order_summary': {
      const cartVarKey = typeof cfg.cart_var_key === 'string' ? cfg.cart_var_key : '';
      return cartVarKey ? `vars.${cartVarKey} → order` : null;
    }
    case 'numeric_menu': {
      const text = typeof cfg.prompt_text === 'string' ? cfg.prompt_text : '';
      const options = Array.isArray(cfg.options)
        ? (cfg.options as Array<Record<string, unknown>>)
        : [];
      const count = options.length;
      const countStr = t ? t('options', { count }) : `${count} option${count === 1 ? '' : 's'}`;
      if (text.length > 0) {
        return count > 0 ? `${truncate(text, 40)} · ${countStr}` : truncate(text);
      }
      return count > 0 ? countStr : null;
    }
  }
}
