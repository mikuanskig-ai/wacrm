 ...
 *     for multi-outgoing types (condition, numeric_menu) so the user
 *     picks which branch they're wiring.
...
function FlowNodeCard({ data, selected }: NodeProps) {
  ...
  // edge of the card. Multi-slot nodes (condition, numeric_menu)
  // render slot rows inline so each handle visually sits next to the
  // slot it represents.
...
function NodeEditSheet({
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      {/* Keyed by node_key so switching the selected node remounts this
          (fresh `showAdvanced` state) instead of leaking the previous
          node's disclosure open/closed — the outer Sheet stays mounted
          so its open/close transition is unaffected. */}
      <NodeEditSheetContent
        key={node.node_key}
        node={node}
        isEntry={isEntry}
        allNodes={allNodes}
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-3 overflow-y-auto px-5 py-4">
        <NodeConfigForm
          ...
          onUpdateConfig={onUpdateConfig}
        />
        {/* Internal node_key — hidden by default like the list view's
            own disclosure (UX audit: canvas showed this unconditionally
            in the header, list view already gated it). */}
        <div className="border-border border-t pt-3">
          ...
          {showAdvanced && (
            <p className="mt-2 flex items-center gap-2 text-xs">
              <span className="text-muted-foreground">{t('nodeKeyLabel')}</span>
