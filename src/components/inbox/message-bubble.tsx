...
/** WhatsApp-style clickable poster — tapping opens the lightbox, which
 *  handles actual playback (controls + autoplay + sound). Kept
 *  `muted` so the browser will render a first-frame poster without
 *  requiring a user gesture first. */
function MediaVideoThumb({ url, onOpen }: { url: string; onOpen: () => void }) {
  return (
    <button
      type="button"
      onClick={onOpen}
...
function MessageContent({
  ...
  onOpenMedia: (kind: "image" | "video", url: string) => void;
}) {
        ...
        <div>
          {message.media_url ? (
            <MediaImage
              ...
              alt="Shared image"
              onOpen={() => onOpenMedia("image", message.media_url!)}
            />
          ) : (
      ...
      return (
        <div>
          {message.media_url ? (
            <MediaVideoThumb
              url={message.media_url}
              onOpen={() => onOpenMedia("video", message.media_url!)}
            />
          ) : (
    ...
    >
      ...
      >
        ...
        <MessageContent
          message={message}
          t={t}
          onOpenMedia={(kind, url) => setLightbox({ kind, url })}
        />
        <div
          className={cn(
