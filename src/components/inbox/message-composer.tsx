...
interface MessageComposerProps {
  ...
  onClearReply?: () => void;
  /**
   * Hides template + interactive (button/list) message actions when
   * 'wuzapi' — those are Meta Cloud API concepts with no equivalent
   * on a self-hosted WuzAPI connection. Defaults to 'meta' so every
   * existing caller keeps showing all actions.
   */
  channelType?: 'meta' | 'wuzapi';
}
...
export function MessageComposer({
  ...
  return (
    <div className="border-t border-border bg-card p-3">
      ...
      {sessionExpired && channelType === 'meta' && (
