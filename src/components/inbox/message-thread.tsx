...
export function MessageThread({
  ...
  // Pending/closed tickets are view-only — the composer is swapped for an
  // attendance-action button in the JSX below, but these guards are the
  // real enforcement (defense in depth, same belt-and-suspenders style as
  // the server-side check in sendMessageToConversation).
  ...
  // The ONLY path that moves a ticket between PENDENTE/CHATBOT → ABERTO →
  // FECHADO (→ PENDENTE on reopen) — goes through the server route so
  // opened_at/opened_by/closed_at/closed_by get stamped consistently.
  // Direct client-side status writes were removed for this reason.
  const [attendanceBusy, setAttendanceBusy] = useState(false);
  const handleAttendanceAction = useCallback(
    async (action: "start" | "close" | "reopen") => {
      if (!conversation) return;
      setAttendanceBusy(true);
      try {
        const res = await fetch(
          `/api/conversations/${conversation.id}/attendance`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        const data = await res.json().catch(() => ({}));
        if (!res.ok) {
          toast.error(data?.error ?? t("attendanceUpdateError"));
          return;
        }
        const status = data.status as ConversationStatus;
        onStatusChange(conversation.id, status);
        // "start" self-assigns when unassigned (see the route) — patch it
        // optimistically too; the realtime UPDATE reconciles either way.
        if (action === "start" && !conversation.assigned_agent_id && user?.id) {
          onAssignChange(conversation.id, user.id);
        }
        toast.success(
          action === "start"
            ? t("startAttendanceSuccess")
            : action === "close"
              ? t("closeAttendanceSuccess")
              : t("reopenSuccess"),
        );
      } catch {
        toast.error(t("attendanceUpdateError"));
      } finally {
        setAttendanceBusy(false);
      }
    },
    [conversation, onStatusChange, onAssignChange, user?.id, t],
  );
  ...
  const postReaction = useCallback(
    async (messageId: string, emoji: string) => {
      ...
      try {
        ...
        const reason = err instanceof Error ? err.message : "network error";
        toast.error(`Reaction not available on this channel: ${reason}`);
    ...
    <div className={cn("flex min-w-0 flex-1 flex-col", DOODLE_BG_CLASSES)}>
      ...
      <div className="flex items-center justify-between gap-2 border-b border-border bg-card px-3 py-3 sm:px-4">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          {/* Clicking the avatar/name opens the contact panel (never
              closes it — a click here should only ever reveal contact
              info, not toggle it away). Reuses the same
              onToggleContactPanel the dedicated button (below) drives,
              so there's a single source of truth for the open/closed
              state. No-ops if the parent didn't wire the toggle up. */}
          <button
            type="button"
            onClick={() => {
              if (!contactPanelOpen) onToggleContactPanel?.();
            }}
            title={onToggleContactPanel ? t("showContact") : undefined}
            className={cn(
              "flex min-w-0 items-center gap-2 rounded-md py-0.5 pr-2 text-left transition-colors sm:gap-3",
              onToggleContactPanel && "hover:bg-muted/60",
            )}
          >
            <Avatar className="h-9 w-9 flex-shrink-0 text-sm font-medium text-foreground">
        ...
        <div className="flex items-center gap-2">
          ...
          {conversation.status === "pending" && (
            <button
              ...
              className="inline-flex items-center justify-center h-7 gap-1 px-2 text-xs rounded-md text-primary hover:bg-muted disabled:opacity-60"
            >
              {attendanceBusy ? (
          ...
          {conversation.status === "open" && (
            <button
              onClick={() => handleAttendanceAction("close")}
              disabled={attendanceBusy}
              className="inline-flex items-center justify-center h-7 gap-1 px-2 text-xs rounded-md text-muted-foreground hover:bg-muted hover:text-foreground disabled:opacity-60"
            >
              {attendanceBusy ? (
                <Loader2 className="h-3 w-3 animate-spin" />
              ) : (
          ...
          {conversation.status === "closed" && (
            <button
              onClick={() => handleAttendanceAction("reopen")}
              disabled={attendanceBusy}
              className="inline-flex items-center justify-center h-7 gap-1 px-2 text-xs rounded-md text-muted-foreground hover:bg-muted hover:text-foreground disabled:opacity-60"
            >
              {attendanceBusy ? (
      ...
      {/* Composer — swapped for a big attendance-action button on
          ...
          is open (see canReply / the handleSend* guards above). */}
      {canReply ? (
        <MessageComposer
          conversationId={conversation.id}
        ...
        <div className="flex flex-col items-center gap-2 border-t border-border bg-card px-4 py-4">
          <button
            type="button"
            onClick={() =>
              handleAttendanceAction(
                conversation.status === "closed" ? "reopen" : "start",
              )
            }
            disabled={attendanceBusy}
            className={cn(
              "inline-flex items-center justify-center gap-2 rounded-md px-6 py-2.5 text-sm font-medium text-primary-foreground transition-colors disabled:opacity-60",
              conversation.status === "closed"
