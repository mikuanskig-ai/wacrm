...
interface Profile {
  ...
  /** Platform-operator flag (migration 062) — orthogonal to
   *  account_role, only ever true for the product owner. Gates the
   *  `/admin` cross-tenant panel. */
...
interface AccountSummary {
  ...
  /** Opt-in feature modules (migration 041) — check with `hasModule()`
   *  from `@/lib/accounts/modules` rather than reading this directly. */
  enabled_modules: string[];
  /** 'active' | 'suspended' (migration 062). A suspended account is
   *  blocked everywhere — see src/lib/accounts/suspension.ts. */
  status: string;
}
...
interface AuthContextValue {
  ...
  /** True if this profile is the platform operator (migration 062). Gates `/admin`. */
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  }, [profile?.account_role, profile?.account_id, profile?.is_platform_admin, account?.status]);
