...
// Mock the service-role client factory. requireApiKey mostly just
// stashes the returned client in the context, but it also queries
// `accounts.status` (isAccountSuspended) before returning — the fake
// `.from('accounts')` chain below reads a mutable module-level flag so
// individual tests can flip the account to 'suspended'. Defaults to
// active so the pre-existing tests are unaffected.
let mockAccountStatus = "active";
vi.mock("@/lib/flows/admin-client", () => ({
  supabaseAdmin: () => ({
    ...
    from: (_table: string) => ({
      select: () => ({
        eq: () => ({
          maybeSingle: () => Promise.resolve({ data: { status: mockAccountStatus }, error: null }),
        }),
      }),
    }),
  }),
...
describe("requireApiKey", () => {
  ...
  it("401s on a token that doesn't look like a ZonTalk key", async () => {
  ...
  it("403s when the key's account is suspended", async () => {
    findActiveKeyByHash.mockResolvedValue(row());
