...
const ORDER_SUMMARY_ADD_MORE_REPLY_ID = "order_summary_add_more";
const ORDER_SUMMARY_FINISH_REPLY_ID = "order_summary_finish";
...
export async function loadProductWithAddonGroups(
  ...
  const typedProduct = product as { id: string; name: string; price: number };
  return {
    id: typedProduct.id,
...
async function sendAddOrderItemProductPrompt(
  ...
  const rows = (products ?? []) as { id: string; name: string; price: number }[];
  ...
  const { whatsapp_message_id } = await engineSendInteractiveList({
    ...
    sections: [
      {
        rows: rows.map((p) => ({
          id: `${AOI_PRODUCT_REPLY_PREFIX}${p.id}`,
          title: p.name.slice(0, 24),
          description: formatCurrency(p.price, currency),
        })),
      },
    ],
  });
...
async function sendAddOrderItemGroupPrompt(
  ...
  const optionRows = group.options.map((o) => ({
    ...
    title: `${selectedIds.has(o.id) ? "✅ " : ""}${o.name}`.slice(0, 24),
    description: o.price_delta !== 0 ? formatCurrency(o.price_delta, currency) : undefined,
  }));
  const controlRows: { id: string; title: string }[] = [];
  if (group.selection_type === "multiple") {
    controlRows.push({ id: `${AOI_DONE_REPLY_PREFIX}${group.id}`, title: "✅ Done" });
  }
  if (!group.is_required) {
    controlRows.push({ id: `${AOI_SKIP_REPLY_PREFIX}${group.id}`, title: "Skip" });
  }
  ...
  const { whatsapp_message_id } = await engineSendInteractiveList({
    ...
    conversationId: run.conversation_id!,
    contactId: run.contact_id!,
    bodyText: `${product.name} — ${group.name}${group.is_required ? " *" : ""}`,
    buttonLabel: "Choose",
    sections: [{ rows: [...optionRows, ...controlRows] }],
  });
  await logEvent(db, run.id, "message_sent", node.node_key, {
    node_type: "add_order_item",
    step: "addon_group",
...
async function sendOrderSummaryPrompt(
  ...
  const { whatsapp_message_id } = await engineSendInteractiveButtons({
    ...
    bodyText,
    buttons: [
      { id: ORDER_SUMMARY_ADD_MORE_REPLY_ID, title: "Add another item" },
      { id: ORDER_SUMMARY_FINISH_REPLY_ID, title: "Finish order" },
    ],
  });
  await logEvent(db, run.id, "message_sent", node.node_key, {
    node_type: "order_summary",
...
async function advanceFromNodeKey(
  ...
  for (let safety = 0; safety < 64; safety += 1) {
    ...
    if (node.node_type === "add_order_item") {
      ...
      const sent = await sendAddOrderItemProductPrompt(db, run, node, cfg);
      if (!sent) {
        await logEvent(db, run.id, "error", node.node_key, {
...
async function handleReplyForActiveRun(
  ...
  // its own early return. `null` means the tap wasn't recognised for
  // the node's current sub-step; fall through to the normal
  // fallback/reprompt policy same as any other unmatched reply.
  if (message.kind === "interactive_reply" && currentNode.node_type === "add_order_item") {
    const result = await handleAddOrderItemReply(
      db,
      run,
      currentNode,
      message.reply_id,
  ...
  //   3. Button tap on an order_summary node — "finish" additionally
  //      creates the delivery_orders row (side effect) before the
  //      normal matched→advance codepath below picks it up.
  //
  ...
  if (
    ...
    if (message.reply_id === ORDER_SUMMARY_ADD_MORE_REPLY_ID) {
      matched = cfg.add_more_next_node_key;
    } else if (message.reply_id === ORDER_SUMMARY_FINISH_REPLY_ID) {
