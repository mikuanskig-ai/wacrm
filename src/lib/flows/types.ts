...
export interface AddOrderItemNodeConfig {
   ...
   * `CartLineItem` — see lib/delivery/create-order.ts) is stored in
   * `flow_runs.vars`. Share the same key across every `add_order_item`
   * node in a flow so multiple category pickers feed one cart.
   */
  cart_var_key: string;
...
/**
 * Zontalk Delivery — reads the cart accumulated by `add_order_item`
 * node(s), shows an itemized summary + total, and offers two buttons:
 * add another item (loops back to an `add_order_item` node) or finish
 * (creates the `delivery_orders` row via `finalizeDeliveryOrder` and
 * advances). See `finishOrderFromSummary` in engine.ts.
 */
export interface OrderSummaryNodeConfig {
  /** Same cart_var_key as the add_order_item node(s) feeding this cart. */
  cart_var_key: string;
  /** Optional text shown before the itemized summary + total. */
  intro_text?: string;
  /** Node to advance to when the customer taps "add another item" — typically an add_order_item node, forming a loop. */
  add_more_next_node_key: string;
  /** Node to advance to after the order is created. */
  finish_next_node_key: string;
  /**
   * Optional — the `flow_runs.vars` key holding the customer's
   * delivery address, typically captured by an earlier `collect_input`
   * node. When set, a real delivery fee is calculated (Motor de
   * Cálculo de Entrega) before the order is created; a failed
   * calculation (out of range, address not found, ...) sends a
   * deterministic message and keeps the run suspended here instead of
   * finishing the order. Omitted = no fee is calculated, same
...
/**
 * Sends a plain-text numbered menu — the only branching prompt the
 * builder offers, now that Meta's interactive buttons/list messages
 * (send_buttons/send_list) have been removed: whatsmeow has no
 * equivalent to those, so this channel-agnostic text menu is what
 * every account uses. The customer replies with the option's number
 * (or, as a fallback, its exact label text) instead of tapping a
 * WhatsApp UI element — see matchNumericMenuReply in engine.ts. No
 * reply_id concept here: matching is purely by array position
 * (rendered as "1: Financeiro", "2: Comercial", …) or by label text.
 *
 * `header_text`/`footer_text` are optional framing lines around the
 * menu — the one bit of visual structure send_list used to offer that
 * a bare numbered list doesn't.
 */
export interface NumericMenuNodeConfig {
  /** Intro text sent before the numbered option list. */
  prompt_text: string;
  /** Optional line rendered above prompt_text (bold framing, no WhatsApp markup enforced). */
  header_text?: string;
  /** Optional line rendered after the numbered options. */
  footer_text?: string;
  /** 1+ options, numbered 1..N by array order. */
  options: Array<{
    /** Visible label, both in the rendered menu and as a fallback match. */
    label: string;
    /** node_key the runner advances to when this option is picked. */
    next_node_key: string;
  }>;
}
...
export type FlowNodeConfig =
  ...
  | { node_type: "add_order_item"; config: AddOrderItemNodeConfig }
  | { node_type: "order_summary"; config: OrderSummaryNodeConfig }
  | { node_type: "numeric_menu"; config: NumericMenuNodeConfig };
