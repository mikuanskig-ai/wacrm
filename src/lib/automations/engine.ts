...
export interface AutomationContext {
  ...
  agent_id?: string
  /** New status, for order_status_changed trigger. */
  order_status?: string
  /** New payment status, for order_payment_status_changed trigger. */
...
async function runStep(step: AutomationStep, args: ExecuteArgs): Promise<string> {
  ...
  switch (step.step_type) {
    case 'send_message': {
      ...
      return `sent via WuzAPI (${whatsapp_message_id})`
    }
...
export function triggerMatches(automation: Automation, ctx: AutomationContext | undefined): boolean {
  ...
  // Only fires when the order's new status matches the configured
  // target status — not on every status change — so "notify when
  // ready" doesn't also fire on "confirmed"/"in_production"/etc.
  if (automation.trigger_type === 'order_status_changed') {
    const cfg = automation.trigger_config as OrderStatusTriggerConfig
    const status = ctx?.order_status
    return Boolean(status && cfg?.status && cfg.status === status)
  }

  // Same reasoning as order_status_changed: only fires on the
  // configured target payment status, not every payment update.
  if (automation.trigger_type === 'order_payment_status_changed') {
    const cfg = automation.trigger_config as PaymentStatusTriggerConfig
    const paymentStatus = ctx?.payment_status
    return Boolean(paymentStatus && cfg?.payment_status && cfg.payment_status === paymentStatus)
  }
