...
export function validateTriggerForActivation(
  ...
  if (triggerType === 'keyword_match') {
    ...
    if (!nonEmpty(cfg.status)) {
      issues.push({ path: 'trigger.status', message: 'order status is required' })
    }
  } else if (triggerType === 'order_payment_status_changed') {
    if (!nonEmpty(cfg.payment_status)) {
      issues.push({ path: 'trigger.payment_status', message: 'payment status is required' })
    }
  }
