...
describe("triggerMatches — order_status_changed", () => {
  function automation(status?: string): Automation {
    return {
      id: "a1",
      account_id: ACCOUNT,
      user_id: "u1",
  ...
  it("matches only the exact configured status", () => {
    expect(triggerMatches(automation("ready"), { order_status: "ready" })).toBe(true);
    expect(triggerMatches(automation("ready"), { order_status: "confirmed" })).toBe(false);
  });

  it("fails closed when the config or event status is missing", () => {
    expect(triggerMatches(automation(), { order_status: "ready" })).toBe(false);
    expect(triggerMatches(automation("ready"), {})).toBe(false);
    expect(triggerMatches(automation("ready"), undefined)).toBe(false);
  });
});

describe("triggerMatches — order_created", () => {
  it("always matches — unconditional trigger, no config to check", () => {
...
describe("triggerMatches — order_payment_status_changed", () => {
  function automation(paymentStatus?: string): Automation {
    return {
      id: "a1",
      account_id: ACCOUNT,
      user_id: "u1",
      name: "payment approved notice",
      trigger_type: "order_payment_status_changed",
      trigger_config: paymentStatus ? { payment_status: paymentStatus } : {},
      is_active: true,
      execution_count: 0,
      created_at: "",
      updated_at: "",
  ...
  it("matches only the exact configured payment status", () => {
    expect(triggerMatches(automation("approved"), { payment_status: "approved" })).toBe(true);
    expect(triggerMatches(automation("approved"), { payment_status: "rejected" })).toBe(false);
  });

  it("fails closed when the config or event payment status is missing", () => {
    expect(triggerMatches(automation(), { payment_status: "approved" })).toBe(false);
    expect(triggerMatches(automation("approved"), {})).toBe(false);
    expect(triggerMatches(automation("approved"), undefined)).toBe(false);
  });
});
