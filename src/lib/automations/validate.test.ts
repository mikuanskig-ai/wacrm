...
describe("validateTriggerForActivation", () => {
  ...
  it("requires status on order_status_changed triggers", () => {
    expect(validateTriggerForActivation("order_status_changed", {})).toEqual([
      { path: "trigger.status", message: "order status is required" },
    ]);
    expect(
      validateTriggerForActivation("order_status_changed", { status: "ready" }),
    ).toEqual([]);
  });

  it("requires no config on order_created triggers", () => {
    expect(validateTriggerForActivation("order_created", {})).toEqual([]);
  });

  it("requires payment_status on order_payment_status_changed triggers", () => {
    expect(validateTriggerForActivation("order_payment_status_changed", {})).toEqual([
      { path: "trigger.payment_status", message: "payment status is required" },
    ]);
    expect(
      validateTriggerForActivation("order_payment_status_changed", { payment_status: "approved" }),
    ).toEqual([]);
  });
});
