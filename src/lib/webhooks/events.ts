...
export const WEBHOOK_EVENTS = [
  ...
  'order.created', // a Delivery order was placed (manual or via WhatsApp Flow)
  'order.status_changed', // a Delivery order's status ladder advanced
  'order.payment_updated', // a Delivery order's payment status changed (Mercado Pago)
] as const;
...
export const WEBHOOK_EVENT_DESCRIPTIONS: Record<WebhookEvent, string> = {
  ...
  'order.created': 'A Delivery order was placed',
  'order.status_changed': "A Delivery order's status changed",
  'order.payment_updated': "A Delivery order's payment status changed",
};
