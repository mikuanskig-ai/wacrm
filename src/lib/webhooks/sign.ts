...
//
// Every delivery carries an `X-Zontalk-Signature` header so receivers
// can verify the request really came from ZonTalk and wasn't tampered
// with or replayed. The scheme is Stripe-style:
//
//   X-Zontalk-Signature: t=<unix_seconds>,v1=<hex HMAC-SHA256>
//
...
/**
 * Build the `X-Zontalk-Signature` header value for `rawBody`, signed
...
/**
 * Verify a signature header. Exposed so a ZonTalk-to-ZonTalk integration
