...
/**
 * Null when the slug is unset/unknown OR the account doesn't have the
 * `delivery` module enabled — deliberately the same result for both,
 * so a 404 built on top of this never leaks which case it was.
 */
...
export async function loadPublicMenu(
  ...
  const { data: productRows } = await db
    ...
    .select('id, category_id, name, description, price, image_url, position')
    .eq('account_id', accountId)
    .eq('is_active', true)
    .order('position');
