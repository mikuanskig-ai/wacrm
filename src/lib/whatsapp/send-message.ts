...
export async function sendMessageToConversation(
  ...
  // go through this function (they use lib/flows/meta-send.ts and
  // lib/automations/meta-send.ts instead), so this doesn't block the
  ...
  const isWuzapi = config.channel_type === 'wuzapi';

  // wuzapi rows carry the connection token in `wuzapi_token`, not
  // `access_token` — decrypt the field that's actually populated for
  // this row's channel_type (the schema CHECK in migration 037
  // guarantees exactly one of the two is set).
  const accessToken = isWuzapi
    ? decrypt(config.wuzapi_token)
    : decrypt(config.access_token);

  // Self-heal legacy CBC ciphertexts. Fire-and-forget; idempotent.
  // Meta-only — wuzapi_token is always written in GCM format (no
  // legacy CBC rows can exist for a column that didn't exist before
  // GCM became the only format).
  if (!isWuzapi && isLegacyFormat(config.access_token)) {
    void db
      .from('whatsapp_config')
  ...
  // wuzapi/whatsmeow has no concept of Meta's pre-approved templates or
  // interactive button/list messages — surface a clear error rather
  // than letting the request fall through to the Meta-shaped builder
  // below. Settings UI hides these options when wuzapi is the active
  // channel; this is the server-side backstop for direct API callers
  // (public /api/v1/messages).
  if (isWuzapi && (messageType === 'template' || messageType === 'interactive')) {
    throw new SendMessageError(
      'unsupported_for_channel',
      `message_type "${messageType}" is not supported on the WuzAPI channel (Meta-only feature).`,
      400
    );
  }

  // WuzAPI takes media inline as a base64 data URI rather than a
  // fetchable link — resolve `mediaUrl`'s bytes once up front so
  // `attempt()` can retry (in principle) without re-downloading.
  const wuzapiMediaDataUri = isWuzapi && isMediaKind
    ? await (async () => {
  ...
  const attempt = async (phone: string): Promise<string> => {
    if (isWuzapi) {
      // wuzapi_base_url is guaranteed set for a wuzapi-channel row by
      // the migration 037 CHECK constraint.
  ...
  // Send. Meta gets the phone-variant retry loop (its sandbox rejects
  // some E.164 forms with "recipient not in allowed list"); wuzapi
  // gets a single attempt — whatsmeow resolves numbers directly
  // against WhatsApp's own directory, so there's no sandbox-allowlist
  // failure mode to retry around.
  let waMessageId = '';
  ...
  try {
    ...
    throw new SendMessageError(
      isWuzapi ? 'wuzapi_error' : 'meta_error',
      `${isWuzapi ? 'WuzAPI' : 'Meta API'} error: ${message}`,
      502
    );
  }
