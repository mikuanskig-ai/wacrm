...
export async function resolveConversationByPhone(
  ...
  if (existing) {
    ...
    if (createErr || !created) {
      ...
      contactCreated = true;
      await autoAddContactsToPipelines(db, accountId, ownerUserId, [
        { id: contactId, name: name || null, phone: sanitized },
      ]);
    }
  }
...
async function findOrCreateConversationRow(
  ...
  // A public-API-initiated send is business-initiated, same as the
  // dashboard's Contact-detail send — pre-open it (mirroring the
  // /attendance "start" action) rather than leaving it at the inbound-
  // webhook default of 'pending', which sendMessageToConversation would
  // otherwise reject with conversation_not_open on the send that follows.
