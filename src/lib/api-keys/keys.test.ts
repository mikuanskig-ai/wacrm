...
describe('hashApiKey', () => {
  it('is deterministic', () => {
    expect(hashApiKey('zontalk_live_abc')).toBe(hashApiKey('zontalk_live_abc'));
  });

  it('differs for different inputs', () => {
    expect(hashApiKey('zontalk_live_abc')).not.toBe(hashApiKey('zontalk_live_abd'));
  });
});
...
describe('timingSafeHexEqual', () => {
  it('is true for identical digests', () => {
    const h = hashApiKey('zontalk_live_xyz');
