...
describe('generateReply — Groq', () => {
  it('calls Groq\'s own chat completions endpoint', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      okResponse({ choices: [{ message: { content: 'Fast reply!' } }] }),
    )
    vi.stubGlobal('fetch', fetchMock)
...
describe('generateReply — OpenRouter', () => {
  it('calls OpenRouter\'s own chat completions endpoint with the underlying model id', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      okResponse({ choices: [{ message: { content: 'Routed reply!' } }] }),
    )
    vi.stubGlobal('fetch', fetchMock)

    const res = await generateReply({
      config: config({ provider: 'openrouter', model: 'anthropic/claude-sonnet-5', apiKey: 'sk-or-test' }),
      systemPrompt: 'sys',
      messages: [{ role: 'user', content: 'Hi' }],
    })

    expect(res.text).toBe('Routed reply!')
    const [url, opts] = fetchMock.mock.calls[0]
    expect(url).toBe('https://openrouter.ai/api/v1/chat/completions')
    expect(JSON.parse(opts.body).model).toBe('anthropic/claude-sonnet-5')
  })
})
...
describe('generateReplyWithTools', () => {
  ...
  it('routes the tool loop to Groq\'s own endpoint', async () => {
    const execute = vi.fn().mockResolvedValue({ content: 'It is sunny.' })
    const tool = fakeTool('get_weather', execute)
    ...
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(openAiToolCallResponse([{ id: 'call_1', name: 'get_weather', args: { city: 'SP' } }]))
      .mockResolvedValueOnce(okResponse({ choices: [{ message: { content: 'Sunny!' } }] }))
    vi.stubGlobal('fetch', fetchMock)
  ...
  it('routes the tool loop to OpenRouter\'s own endpoint', async () => {
    const execute = vi.fn().mockResolvedValue({ content: 'It is sunny.' })
    const tool = fakeTool('get_weather', execute)
