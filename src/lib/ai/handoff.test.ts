...
describe('buildHandoffSummary', () => {
  ...
  it('uses a distinct message for a provider-error handoff', () => {
    const summary = buildHandoffSummary({
      messages: [{ role: 'user', content: 'quero pedir uma pizza' }],
      replyCount: 0,
      reason: 'provider_error',
    })
    expect(summary).toContain('⚠️ AI agent handed off after a provider error')
    expect(summary).not.toContain('🤖 AI agent handed off after')
    expect(summary).toContain('“quero pedir uma pizza”')
  })
})
