...
export const RATE_LIMITS = {
  ...
  /** Public cardápio menu read (`/api/public/menu/[slug]`, no auth,
   *  per-IP). 60/min lets a visitor browse/reload the page freely
   *  while bounding a scraping loop. */
  publicMenuRead: { limit: 60, windowMs: 60_000 },
  /** Public cardápio order creation (`/api/public/menu/[slug]/order`,
   *  no auth, per-IP). Tighter than the read side — this is a write
   *  with zero authentication, the first of its kind in this
   *  codebase, so it gets the same order of magnitude as
   *  `invitationRedeem`. */
  publicMenuOrder: { limit: 10, windowMs: 60_000 },
  /** Public cardápio delivery-fee preview
   *  (`/api/public/menu/[slug]/delivery-fee`, no auth, per-IP). A live
   *  preview fires on every address edit, so it's more permissive than
   *  the order write itself but still bounded. */
  publicMenuCalculate: { limit: 30, windowMs: 60_000 },
  /** Public pricing page read (`/api/public/plans`, no auth, per-IP).
   *  Same order of magnitude as `publicMenuRead` — a visitor browsing
   *  /pricing, not a write. */
  publicPlansRead: { limit: 60, windowMs: 60_000 },
  /** InfinitePay billing webhook (`/api/billing/webhook`, no auth,
   *  per-IP). Unlike the Mercado Pago webhook, InfinitePay documents
   *  no signature scheme — every POST here triggers an outbound
   *  `payment_check` call to the gateway, so this is tighter than a
   *  typical public-read bucket to bound that amplification. */
  billingWebhook: { limit: 20, windowMs: 60_000 },
} as const;
