...
const mocks = vi.hoisted(() => ({
  ...
  calculateDeliveryFeeForAccount: vi.fn(),
}));
...
vi.mock('@/lib/delivery/create-order', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@/lib/delivery/create-order')>();
  return { ...actual, finalizeDeliveryOrder: mocks.finalizeDeliveryOrder };
});

vi.mock('@/lib/delivery/fee-engine', () => ({
  calculateDeliveryFeeForAccount: mocks.calculateDeliveryFeeForAccount,
}));
...
beforeEach(() => {
  ...
  mocks.calculateDeliveryFeeForAccount.mockReset();
  ...
  mocks.calculateDeliveryFeeForAccount.mockResolvedValue({
    ok: true,
    fee: 8,
    distanceKm: null,
    freeShipping: false,
...
describe('POST /api/public/menu/[slug]/order', () => {
  ...
  it('rejects with 422 when the delivery fee engine reports the address is out of range', async () => {
    mocks.calculateDeliveryFeeForAccount.mockResolvedValue({ ok: false, reason: 'out_of_range' });
    const res = await POST(
      request({
        items: [{ product_id: 'p1', quantity: 1, addon_option_ids: ['o1'] }],
        delivery_address: 'Muito longe, 999',
      }),
      params,
    );
    ...
    expect(mocks.finalizeDeliveryOrder).not.toHaveBeenCalled();
    const body = await res.json();
    expect(body).toEqual({ error: 'delivery_fee_unavailable', reason: 'out_of_range' });
  });

  it('passes the computed delivery fee through to finalizeDeliveryOrder', async () => {
    mocks.calculateDeliveryFeeForAccount.mockResolvedValue({
      ok: true,
      fee: 12,
      distanceKm: 4.2,
    ...
    await POST(
      request({
        items: [{ product_id: 'p1', quantity: 1, addon_option_ids: ['o1'] }],
        delivery_address: 'Rua Y, 456',
      }),
      params,
    );

    const call = mocks.finalizeDeliveryOrder.mock.calls[0][1];
    expect(call.deliveryFee).toBe(12);
    expect(mocks.calculateDeliveryFeeForAccount).toHaveBeenCalledWith(
      expect.anything(),
      'acc-1',
      expect.objectContaining({ address: 'Rua Y, 456', subtotal: 53 }),
    );
  });
});
