...
async function findOrCreateConversation(
  ...
  // A business-initiated conversation (agent messaging a contact who has
  // no thread yet) is, by construction, already "attended" — the agent
  // is the one starting it right here. Create it pre-opened (mirroring
  // the /attendance "start" action) instead of the inbound-webhook
  // default of 'pending', which would otherwise make the send that
  // follows immediately get rejected by sendMessageToConversation's
  // status !== 'open' guard.
