        ...
        case 'conversations':
          return {
            data: {
              ...
              contact: CONTACT,
              // The route pre-opens a business-initiated conversation
              // (mirrors the /attendance "start" action) so the send
              // that follows isn't rejected by sendMessageToConversation's
              // status !== 'open' guard.
              status: 'open',
            },
            error: null,
...
describe('POST /api/whatsapp/send — contact_id template path', () => {
  ...
  it('reuses an existing conversation instead of creating a duplicate', async () => {
    existingConversation = {
      ...
      contact_id: 'contact-1',
      contact: CONTACT,
      // Reused thread must already be open, or the send core's
      // status !== 'open' guard would reject it — this test is about
      // dedup, not attendance-state gating.
      status: 'open',
    }
