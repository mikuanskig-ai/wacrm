...
async function parseWuzapiEvent(
...
): Promise<{ message: WhatsAppMessage; content: ParsedContent; pushName: string } | null> {
  if (payload.type !== 'Message' || !payload.event?.Info || !payload.event.Message) {
    return null
  }
  const { Info, Message } = payload.event
  if (Info.IsFromMe) return null // ignore our own messages echoed back
  ...
  // number at all, which would corrupt contact matching.
  const from = bareJidToPhone(Info.SenderAlt || Info.Sender)
  const pushName = Info.PushName || from
...
export async function POST(request: Request) {
  ...
  after(async () => {
    try {
      ...
      const { message, content, pushName } = parsed
      ...
      await processMessage(
        ...
        config.account_id,
        config.user_id,
        '', // accessToken unused — precomputedContent bypasses parseMessageContent's Meta media-verify path
        resolvedContent,
      )
    } catch (error) {
