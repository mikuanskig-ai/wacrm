...
import { getAccountPlan, countSeats, canInvite } from "@/lib/billing/plans";
...
export async function POST(request: Request) {
  try {
    ...
    // Plan seat limit — counts current members + this account's own
    // other outstanding invitations. `plan_id IS NULL` (legacy/no-plan
    // accounts) is unlimited, matching pre-billing behavior.
    const admin = supabaseAdmin();
    const plan = await getAccountPlan(admin, ctx.accountId);
    const seats = await countSeats(admin, ctx.accountId);
    if (!canInvite(plan, seats)) {
      return NextResponse.json(
        { error: `This account's plan is limited to ${plan.maxUsers} users` },
        { status: 403 },
      );
    }
