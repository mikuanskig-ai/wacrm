...
//
// A job reaching here is normally `claimed` (see claim_print_jobs,
// migration 060) — GET /api/v1/print-jobs flips it there atomically
// before handing it to the agent. `pending` is also accepted for any
// job claimed before that migration shipped, so an in-flight job at
// deploy time isn't stranded.
//
// Idempotent: if the job is in neither state when this is called
// (duplicate ack from a flaky agent connection, or a stale reclaim
// already handed it to a different poller), it just returns the
