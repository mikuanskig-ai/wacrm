...
// (scope: print:read)
//
// Consumed by a local print agent (polling, not yet built — see the
// print-queue plan) rather than a human. Not cursor-paginated like
// the other v1 list endpoints: this is a small live operational
// queue, not a browsable history. Returns up to PRINT_JOB_LIMIT
// oldest-first pending jobs plus `pending_count` (the true total, so
// a backlog beyond the page is distinguishable from "caught up").
//
...
export async function GET(request: Request) {
  try {
    ...
    const { data: jobRows, error: jobsErr } = await ctx.supabase
      .from('print_jobs')
      .select('id, order_id, created_at, attempts')
      .eq('account_id', ctx.accountId)
      .eq('status', 'pending')
      .or(dueFilter)
      .order('created_at', { ascending: true })
      .limit(PRINT_JOB_LIMIT);

    if (jobsErr) {
      console.error('[api/v1/print-jobs] list error:', jobsErr);
    ...
    if (orderIds.length > 0) {
      const [ordersRes, itemsRes] = await Promise.all([
        ctx.supabase
          ...
          .select(
            'id, status, source, customer_name, delivery_address, notes, subtotal, delivery_fee, total, currency, created_at, contact:contacts(name, phone)',
          )
          .in('id', orderIds),
        ctx.supabase
