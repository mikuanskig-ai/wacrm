...
export async function GET(request: Request) {
  ...
  // Migration 062 — scheduled steps (e.g. "wait 2 days") for a
  // suspended account must not fire, even though they were enqueued
  // before the account was suspended.
  const suspended = await getSuspendedAccountIds(admin)

  let processed = 0
  for (const row of due) {
    if (suspended.has(row.account_id as string)) continue
