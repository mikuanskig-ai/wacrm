...
const CONTACT_PANEL_STORAGE_KEY = "zontalk:inbox:contact-panel-open";
...
function InboxPageInner() {
  ...
  const handleConversationsLoaded = useCallback(
    (loaded: Conversation[]) => {
      ...
      // Keep the already-open thread's contact snapshot (thread header
      // avatar, ContactSidebar) fresh. activeContact is otherwise only
      // ever set once, at click-time (handleSelectConversation below) —
      // so a contact field that changes while the conversation is
      // already open (e.g. avatar_url backfilling in after the fix for
      // "contact photos aren't showing") would never reach the open
      // thread without a full page reload.
      if (activeConversation?.id) {
        const fresh = loaded.find((c) => c.id === activeConversation.id);
        if (fresh?.contact) setActiveContact(fresh.contact);
      }
  ...
  // Inline-edit in ContactSidebar writes straight to the DB; this just
  // keeps the already-open thread's contact snapshot (header avatar/
  // name, sidebar) in sync immediately instead of waiting for the next
  // conversation-list resync.
