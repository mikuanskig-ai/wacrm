...
// PostgREST's default max-rows setting caps any single request — this
// keeps the export a single request instead of adding pagination just
// for the rare account past that size. Generous enough for the vast
// majority of accounts; matches the "some export beats none" bar this
// closes (UX audit Parte 2 — contacts had no export path at all).
...
export default function ContactsPage() {
  ...
  // not just the loaded page — since "export what I'm looking at" is
  // the useful behavior, and the list is paginated 25-at-a-time.
  const handleExportCsv = useCallback(async () => {
    ...
    try {
      ...
      contactTags?.forEach((ct) => {
        if (!tagIdsByContact[ct.contact_id]) tagIdsByContact[ct.contact_id] = [];
