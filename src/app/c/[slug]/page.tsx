...
import {
  ...
  DELIVERY_FEE_REASON_KEY,
  type CheckoutFormValues,
  type DeliveryFeeReason,
} from '@/components/public-menu/cart-drawer';
...
export default function PublicMenuPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const handleSubmit = async (values: CheckoutFormValues) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.error === 'delivery_fee_unavailable' && data.reason) {
          const reason = data.reason as DeliveryFeeReason;
          toast.error(t(`deliveryFee.${DELIVERY_FEE_REASON_KEY[reason]}`));
        } else {
