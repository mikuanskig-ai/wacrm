-- ============================================================
-- 061_fix_redeem_invitation_dangling_table.sql
--
-- Fixes a real, currently-broken invitation flow: redeem_invitation()
-- (019_invitation_rpcs.sql, last touched by 034) checks the invitee's
-- own about-to-be-deleted personal account for existing data across a
-- list of tables, including `message_templates` — which
-- 054_remove_meta_cloud_api.sql dropped (pure Meta Business-Manager
-- scaffolding) without updating this function. Every redeem attempt
-- since has failed with `relation "message_templates" does not exist`
-- (confirmed live via server logs), silently blocking every pending
-- invitation link.
--
-- Re-creates the function identically, minus the dangling reference.
-- ============================================================
