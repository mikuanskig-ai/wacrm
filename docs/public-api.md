...
Cloud-side queue for a local print agent (not shipped by wacrm — this
is the API a store's own printer-connector program polls) to print
Delivery order receipts automatically instead of a staff member
clicking Print in the dashboard. Enable it under Settings → Auto-print
