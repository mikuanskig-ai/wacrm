...
| `print:read`         | List pending Delivery print jobs (for a local print agent) |
| `print:write`        | Acknowledge Delivery print jobs as printed or failed |
...
Cloud-side queue for a local print agent to print Delivery order
receipts automatically instead of a staff member clicking Print in
the dashboard. The agent itself ships in `print-agent/` at the repo
root — a standalone Windows `.exe` (see `print-agent/README.md`) that
a shop installs on its back-office PC; it polls the two endpoints
below and talks to the receipt printer directly (network socket or a
shared Windows printer). Enable printing under Settings → Impressão
first (opt-in, off by default), then create a key with both scopes
below for the agent to use.

- `GET /api/v1/print-jobs` (scope `print:read`) — returns up to 20
  oldest pending jobs plus `pending_count` (the true total, in case
  there's a backlog beyond the page). Each job embeds a ready-to-print
  receipt — no second request needed per job.

  ```jsonc
  // → { "data": { "account_name": "…", "pending_count": 2, "jobs": [
  {
    "id": "…", "order_id": "…", "created_at": "…", "attempts": 0,
    "receipt": {
      "status": "pending_confirmation", "source": "whatsapp_flow",
      "customer_name": "…", "customer_phone": "…",
      "delivery_address": "…", "notes": "…",
      "subtotal": 45.0, "delivery_fee": 5.0, "total": 50.0, "currency": "BRL",
      "created_at": "…",
      "items": [{ "product_name": "…", "quantity": 1, "unit_price": 45.0, "addons_snapshot": [], "line_total": 45.0, "notes": null }]
    }
  }
  // ] } }
  ```

- `POST /api/v1/print-jobs/{id}/ack` (scope `print:write`) — body
  `{ "status": "printed" | "failed", "error"?: "…" }`. Idempotent: acking a
  job that's no longer `pending` just returns its current state. A
  `failed` ack backs off before the job is served again (60s after
  the 1st failure, 120s after the 2nd); the 3rd failure is terminal
  (`status: "failed"`, no further retries).

A job whose order gets cancelled before the agent serves it is
dropped automatically (marked `skipped`, never returned) — no action
needed on the agent's side.
